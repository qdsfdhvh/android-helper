...
- [x] **③ 设备信息弹窗** ℹ️ — 点击设备名称 → 终端显示含 model/Android 版本/API/arch/
       density/resolution/IP/电池的详细设备信息。

- [x] **④ ADB Shell 快捷输入** 💻 — 底栏小输入框 `$` ,选中设备后输入命令
       按 Enter → `adb shell <命令>` 在新终端执行,同时清空输入框。

- [x] **⑤ Logcat 增强** 🔍 — 新增 Main buffer / System buffer / Crash buffer 选项,
       新增 Custom filter 输入关键词 → `adb logcat | grep -i <keyword>`。

### 🟢 P2 — 低优先级

- [x] **⑥ 强制停止 / 清除数据** 🛑 — 设备行 Install 旁加 `⋯` 按钮 →
       modal 可选 Force stop / Clear data / Uninstall,检测到包名后可用。
